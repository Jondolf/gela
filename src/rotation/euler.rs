...
///
/// ```
/// # use gela::matrix::GMat3;
/// # use gela::rotation::EulerRot;
/// # let i = core::f32::consts::FRAC_PI_2;
/// # let j = core::f32::consts::FRAC_PI_4;
/// # let k = core::f32::consts::FRAC_PI_8;
/// let m_intrinsic = GMat3::from_rotation_x(i) * GMat3::from_rotation_y(j) * GMat3::from_rotation_z(k);
/// let n_intrinsic = GMat3::from_euler(EulerRot::XYZ, i, j, k);
/// assert!(m_intrinsic.abs_diff_eq(n_intrinsic, 2e-6));
///
/// let m_extrinsic = GMat3::from_rotation_z(k) * GMat3::from_rotation_y(j) * GMat3::from_rotation_x(i);
/// let n_extrinsic = GMat3::from_euler(EulerRot::XYZEx, i, j, k);
...
impl<T: ScalarReal> FromEuler for GMat3<T> {
    type Scalar = T;
    fn from_euler_angles(euler: EulerRot, x: T, y: T, z: T) -> Self {
        ...
        GMat3::from_cols_array_2d(&m)
    }
}

impl<T: ScalarReal> FromEuler for GMat4<T> {
    type Scalar = T;
    fn from_euler_angles(euler: EulerRot, x: T, y: T, z: T) -> Self {
        GMat4::from_mat3(GMat3::from_euler_angles(euler, x, y, z))
    }
}
...
impl<T: ScalarReal> ToEuler for GMat3<T> {
    type Scalar = T;
    fn to_euler_angles(self, euler: EulerRot) -> (T, T, T) {
        let order = Order::from_euler(euler);
        let (i, j, k) = order.angle_order();

        let mut ea = GVec3::ZERO;
        if order.initial_repeated {
...
impl<T: ScalarReal> ToEuler for GMat4<T> {
    type Scalar = T;
    fn to_euler_angles(self, order: EulerRot) -> (T, T, T) {
        GMat3::from_mat4(self).to_euler_angles(order)
    }
}
