use crate::rotation::GRot2;
use crate::vector::GVec2;

use core::{iter::Product, ops::*};

use gnum::{
    num::{Float, NumCast, Real},
    simd::Select,
};

#[cfg(feature = "zerocopy")]
use zerocopy_derive::*;

/// A 2D isometry, which can represent rotation followed by translation.
#[derive(Clone, Copy, PartialEq)]
#[cfg_attr(feature = "zerocopy", derive(FromBytes, Immutable, KnownLayout))]
#[repr(C)]
pub struct GIso2<T: Real> {
    /// The rotational part of the isometry.
    pub rotation: GRot2<T>,
    /// The translational part of the isometry.
    pub translation: GVec2<T>,
}

/// # Constants
impl<T: Real> GIso2<T> {
    /// The identity isometry.
    ///
    /// Multiplying a vector with this returns the same vector.
    pub const IDENTITY: Self = Self::from_rotation_translation(GRot2::IDENTITY, GVec2::ZERO);
}

impl<T: Float> GIso2<T> {
    /// All `NAN`.
    pub const NAN: Self = Self::from_rotation_translation(GRot2::NAN, GVec2::NAN);
}

/// # Construction
impl<T: Real> GIso2<T> {
    /// Creates an isometry from a rotation and a translation.
    #[inline(always)]
    #[must_use]
    pub const fn from_rotation_translation(rotation: GRot2<T>, translation: GVec2<T>) -> Self {
        Self {
            rotation,
            translation,
        }
    }

    /// Creates an isometry from the elements in `if_true` and `if_false`,
    /// selecting which to use based on the given `boolean`.
    ///
    /// A true boolean uses the corresponding element from `if_true`, and false
    /// uses the element from `if_false`.
    #[inline]
    #[must_use]
    pub fn select<B: Select<T>>(boolean: B, if_true: Self, if_false: Self) -> Self {
        Self {
            rotation: GRot2::select(boolean, if_true.rotation, if_false.rotation),
            translation: GVec2::select(boolean, if_true.translation, if_false.translation),
        }
    }

    /// Creates an isometry from the given `rotation`, with no translation.
    #[inline]
    #[must_use]
    pub fn from_rotation(rotation: GRot2<T>) -> Self {
        Self {
            rotation,
            translation: GVec2::ZERO,
        }
    }

    /// Creates an isometry from the given 2D `translation`, with no rotation.
    #[inline]
    #[must_use]
    pub fn from_translation(translation: GVec2<T>) -> Self {
        Self {
            rotation: GRot2::IDENTITY,
            translation,
        }
    }
}

/// # Operations
impl<T: Real> GIso2<T> {
    /// Transforms the given 2D point, applying rotation and translation.
    #[inline]
    #[must_use]
    pub fn transform_point2(&self, rhs: GVec2<T>) -> GVec2<T> {
        self.rotation * rhs + self.translation
    }

    /// Transforms the given 2D vector, applying rotation (but NOT translation).
    ///
    /// To also apply translation, use [`Self::transform_point2()`] instead.
    #[inline]
    pub fn transform_vector2(&self, rhs: GVec2<T>) -> GVec2<T> {
        self.rotation * rhs
    }

    /// Return the inverse of this transform.
    #[inline]
    #[must_use]
    pub fn inverse(&self) -> Self {
        let rotation = self.rotation.inverse();
        let translation = -(rotation * self.translation);

        Self {
            rotation,
            translation,
        }
    }

    /// Returns true if the absolute difference of all elements between `self` and `rhs`
    /// is less than or equal to `max_abs_diff`.
    ///
    /// This can be used to compare if two 3x4 matrices contain similar elements. It works
    /// best when comparing with a known value. The `max_abs_diff` that should be used used
    /// depends on the values being compared against.
    ///
    /// For more see
    /// [comparing floating point numbers](https://randomascii.wordpress.com/2012/02/25/comparing-floating-point-numbers-2012-edition/).
    #[inline]
    #[must_use]
    pub fn abs_diff_eq(&self, rhs: Self, max_abs_diff: T) -> T::Bool {
        self.rotation.abs_diff_eq(rhs.rotation, max_abs_diff)
            & self.translation.abs_diff_eq(rhs.translation, max_abs_diff)
    }
}

impl<T: Float> GIso2<T> {
    /// Returns `true` if, and only if, all elements are finite.
    /// If any element is either `NaN`, positive or negative infinity, this will return `false`.
    #[inline]
    #[must_use]
    pub fn is_finite(&self) -> T::Bool {
        self.rotation.is_finite() & self.translation.is_finite()
    }

    /// Returns `true` if any elements are `NaN`.
    #[inline]
    #[must_use]
    pub fn is_nan(&self) -> T::Bool {
        self.rotation.is_nan() | self.translation.is_nan()
    }
}

/// # SIMD Operations
impl<T: Real> GIso2<T>
where
    T::Element: Real,
{
    /// Broadcasts a scalar isometry into a SIMD isometry, filling all lanes with the same value.
    #[inline]
    pub fn broadcast(value: GIso2<T::Element>) -> Self {
        Self {
            rotation: GRot2::broadcast(value.rotation),
            translation: GVec2::broadcast(value.translation),
        }
    }

    /// Extracts the i-th lane of `self`.
    ///
    /// # Panics
    ///
    /// Panics if `i >= T::LANES`.
    #[inline]
    #[must_use]
    pub fn extract(&self, i: usize) -> GIso2<T::Element> {
        GIso2 {
            rotation: self.rotation.extract(i),
            translation: self.translation.extract(i),
        }
    }

    /// Extracts the i-th lane of `self` without bounds checking.
    ///
    /// # Safety
    ///
    /// Undefined behavior if `i >= T::LANES`.
    #[inline]
    #[must_use]
    pub unsafe fn extract_unchecked(&self, i: usize) -> GIso2<T::Element> {
        unsafe {
            GIso2 {
                rotation: self.rotation.extract_unchecked(i),
                translation: self.translation.extract_unchecked(i),
            }
        }
    }

    /// Replaces the i-th lane of `self` with `value`.
    ///
    /// # Panics
    ///
    /// Panics if `i >= T::LANES`.
    #[inline]
    pub fn replace(&mut self, i: usize, value: GIso2<T::Element>) {
        self.rotation.replace(i, value.rotation);
        self.translation.replace(i, value.translation);
    }

    /// Replaces the i-th lane of `self` with `value` without bounds checking.
    ///
    /// # Safety
    ///
    /// Undefined behavior if `i >= T::LANES`.
    #[inline]
    pub unsafe fn replace_unchecked(&mut self, i: usize, value: GIso2<T::Element>) {
        unsafe {
            self.rotation.replace_unchecked(i, value.rotation);
            self.translation.replace_unchecked(i, value.translation);
        }
    }
}

/// # Conversion
impl<T: Real> GIso2<T> {
    /// Casts the elements of `self` to another type.
    #[inline]
    #[must_use]
    pub fn cast<U: Real>(self) -> GIso2<U>
    where
        T: NumCast<U>,
    {
        GIso2::from_rotation_translation(self.rotation.cast(), self.translation.cast())
    }
}

impl<T: Real> Default for GIso2<T> {
    #[inline]
    fn default() -> Self {
        Self::IDENTITY
    }
}

impl<T: Real + Mul<Output = T>> Mul for GIso2<T> {
    type Output = Self;
    #[inline]
    fn mul(self, rhs: GIso2<T>) -> Self {
        Self {
            rotation: self.rotation * rhs.rotation,
            translation: self.rotation * rhs.translation + self.translation,
        }
    }
}

impl<T: Real + Mul<Output = T>> Mul<&GIso2<T>> for GIso2<T> {
    type Output = Self;
    #[inline]
    fn mul(self, rhs: &GIso2<T>) -> Self {
        self.mul(*rhs)
    }
}

impl<T: Real + Mul<Output = T>> Mul<GIso2<T>> for &GIso2<T> {
    type Output = GIso2<T>;
    #[inline]
    fn mul(self, rhs: GIso2<T>) -> GIso2<T> {
        (*self).mul(rhs)
    }
}

impl<T: Real + Mul<Output = T>> Mul<&GIso2<T>> for &GIso2<T> {
    type Output = GIso2<T>;
    #[inline]
    fn mul(self, rhs: &GIso2<T>) -> GIso2<T> {
        (*self).mul(*rhs)
    }
}

impl<T: Real + Mul<Output = T>> MulAssign for GIso2<T> {
    #[inline]
    fn mul_assign(&mut self, rhs: GIso2<T>) {
        *self = self.mul(rhs);
    }
}

impl<T: Real + MulAssign> MulAssign<&GIso2<T>> for GIso2<T> {
    #[inline]
    fn mul_assign(&mut self, rhs: &GIso2<T>) {
        self.mul_assign(*rhs);
    }
}

impl<T: Real + Mul<Output = T>> Mul<GVec2<T>> for GIso2<T> {
    type Output = GVec2<T>;
    #[inline]
    fn mul(self, rhs: GVec2<T>) -> GVec2<T> {
        self.transform_point2(rhs)
    }
}

impl<T: Real + Mul<Output = T>> Mul<&GVec2<T>> for GIso2<T> {
    type Output = GVec2<T>;
    #[inline]
    fn mul(self, rhs: &GVec2<T>) -> GVec2<T> {
        self.mul(*rhs)
    }
}

impl<T: Real + Mul<Output = T>> Mul<GVec2<T>> for &GIso2<T> {
    type Output = GVec2<T>;
    #[inline]
    fn mul(self, rhs: GVec2<T>) -> GVec2<T> {
        (*self).mul(rhs)
    }
}

impl<T: Real + Mul<Output = T>> Mul<&GVec2<T>> for &GIso2<T> {
    type Output = GVec2<T>;
    #[inline]
    fn mul(self, rhs: &GVec2<T>) -> GVec2<T> {
        (*self).mul(*rhs)
    }
}

impl<T: Real> Product<GIso2<T>> for GIso2<T> {
    fn product<I>(iter: I) -> Self
    where
        I: Iterator<Item = Self>,
    {
        iter.fold(Self::IDENTITY, |a, b| a * b)
    }
}

impl<'a, T: Real> Product<&'a GIso2<T>> for GIso2<T> {
    fn product<I>(iter: I) -> Self
    where
        I: Iterator<Item = &'a Self>,
    {
        iter.fold(Self::IDENTITY, |a, &b| a * b)
    }
}

impl<T: Real + core::fmt::Display> core::fmt::Display for GIso2<T> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        if let Some(p) = f.precision() {
            write!(
                f,
                "GIso2 {{ rotation: {:.*}, translation: {:.*} }}",
                p, self.rotation, p, self.translation
            )
        } else {
            write!(
                f,
                "GIso2 {{ rotation: {}, translation: {} }}",
                self.rotation, self.translation
            )
        }
    }
}

impl<T: Real + core::fmt::Debug> core::fmt::Debug for GIso2<T> {
    fn fmt(&self, fmt: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        fmt.debug_struct(stringify!(GIso2))
            .field("rotation", &self.rotation)
            .field("translation", &self.translation)
            .finish()
    }
}
