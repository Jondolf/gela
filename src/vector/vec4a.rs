use crate::vector::{BVec4A, GVec2, GVec3, GVec4};
...
use gnum::{
    ...
    simd::{Reduce, Select, Shuffle4, SimdLike},
};

/// Creates a 4-dimensional vector with `f32` components, using SIMD vector types
/// on supported platforms.
///
/// The type is 16-byte aligned and uses SIMD instructions for some operations,
/// which can provide better performance than [`Vec4`](crate::vector::Vec4),
...
/// A 4-dimensional vector with `f32` components, using SIMD vector types
/// on supported platforms.
///
/// The type is 16-byte aligned and uses SIMD instructions for some operations,
/// which can provide better performance than [`Vec4`](crate::vector::Vec4)
...
#[repr(transparent)]
pub struct Vec4A(f32x4);

/// Wraps a SIMD comparison mask in a [`BVec4A`].
#[inline(always)]
fn mask_to_bvec(m: <f32x4 as SimdLike>::Bool) -> BVec4A {
    BVec4A::from_mask(m)
}
...
impl Vec4A {
    ...
    #[inline]
    #[must_use]
    pub fn select_mask(mask: BVec4A, if_true: Self, if_false: Self) -> Self {
    ...
    #[inline]
    #[must_use]
    pub fn truncate(self) -> GVec3<f32> {
...
impl Vec4A {
    ...
    pub fn min_element(self) -> f32 {
        NumOrd::min(NumOrd::min(NumOrd::min(self.x, self.y), self.z), self.w)
    }
    ...
    pub fn max_element(self) -> f32 {
        NumOrd::max(NumOrd::max(NumOrd::max(self.x, self.y), self.z), self.w)
    }
    ...
    ///
    /// Unlike [`min_element`](Self::min_element), which walks the four components with scalar
    /// comparisons, this reduces them in the register with two shuffles and two `minps`.
    ///
    /// # Floating-Point Types
    ///
    ...
    pub fn min_element_fast(self) -> f32 {
        let m = NumOrd::min_fast(self.0, Shuffle4::shuffle::<2, 3, 0, 1>(self.0));
        let m = NumOrd::min_fast(m, Shuffle4::shuffle::<1, 1, 1, 1>(m));
        SimdLike::extract(&m, 0)
    }

    /// Returns the horizontal maximum of `self`, using [`NumOrd::max_fast`].
    ///
    /// The counterpart to [`min_element_fast`](Self::min_element_fast).
    ///
    /// # Floating-Point Types
    ///
    ...
    pub fn max_element_fast(self) -> f32 {
        let m = NumOrd::max_fast(self.0, Shuffle4::shuffle::<2, 3, 0, 1>(self.0));
        let m = NumOrd::max_fast(m, Shuffle4::shuffle::<1, 1, 1, 1>(m));
        SimdLike::extract(&m, 0)
    }
}
...
impl Vec4A {
    ...
    pub fn dot(self, rhs: Self) -> f32 {
        // `reduce_sum_stable` associates as `(m0 + m2) + (m1 + m3)`, the same grouping
        // `GVec4::<f32>::dot` uses, so the two are bit-identical on every backend.
        Reduce::reduce_sum_stable(self.0 * rhs.0)
    }
    ...
    pub fn element_sum(self) -> f32 {
        // Bit-identical to `GVec4::<f32>::element_sum`, which uses the same pairing.
        Reduce::reduce_sum_stable(self.0)
    }
    ...
    pub fn element_product(self) -> f32 {
        // Bit-identical to `GVec4::<f32>::element_product`, which uses the same pairing.
        Reduce::reduce_product_stable(self.0)
    }
...
impl Vec4A {
    ...
    /// against overflow and underflow. For example, `Vec4A::MAX.midpoint(Vec4A::MAX)` is `Vec4A::MAX`,
    /// but `Vec4A::MAX.midpoint_fast(Vec4A::MAX)` overflows to `Vec4A::INFINITY`.
    #[inline]
    #[must_use]
    ...
    pub fn abs_diff_eq(self, rhs: Self, max_abs_diff: f32) -> bool {
        let d = (self - rhs).abs();
        d.x <= max_abs_diff && d.y <= max_abs_diff && d.z <= max_abs_diff && d.w <= max_abs_diff
    }
...
/// # Conversions
impl Vec4A {
    /// Creates a [`Vec4A`] from a [`GVec4<f32>`].
    #[inline(always)]
    #[must_use]
    pub const fn from_vec4(v: GVec4<f32>) -> Self {
        Self::new(v.x, v.y, v.z, v.w)
    }

    /// Converts `self` to a [`GVec4<f32>`].
    #[inline(always)]
    #[must_use]
    pub fn to_vec4(self) -> GVec4<f32> {
    ...
    #[inline]
    #[must_use]
    pub fn cast<U: Copy>(self) -> GVec4<U>
    where
        f32: NumCast<U>,
    {
        self.to_vec4().cast()
...
impl Vec4A {
    /// Creates a new [`Vec4A`] from its SIMD register.
    #[inline(always)]
...
macro_rules! impl_binary_op {
    ($($trait:ident, $method:ident, $assign_trait:ident, $assign_method:ident);* $(;)?) => {
        $(
            ...
            impl $trait<&Vec4A> for Vec4A {
                type Output = Self;

                #[inline(always)]
                fn $method(self, rhs: &Vec4A) -> Self {
            ...
            impl $trait<Vec4A> for &Vec4A {
                ...
                #[inline(always)]
                fn $method(self, rhs: Vec4A) -> Vec4A {
                    $trait::$method(*self, rhs)
                }
            }
            ...
            impl $trait<Vec4A> for f32 {
                type Output = Vec4A;

                #[inline(always)]
                fn $method(self, rhs: Vec4A) -> Vec4A {
                    Vec4A($trait::$method(f32x4::splat(self), rhs.0))
                }
            }
            ...
            impl $assign_trait<&Vec4A> for Vec4A {
                #[inline(always)]
                fn $assign_method(&mut self, rhs: &Vec4A) {
                    $assign_trait::$assign_method(self, *rhs);
                }
            }

            impl $assign_trait<f32> for Vec4A {
                #[inline(always)]
...
impl<'a> Product<&'a Vec4A> for Vec4A {
    #[inline]
...
impl From<GVec4<f32>> for Vec4A {
    #[inline(always)]
    fn from(v: GVec4<f32>) -> Self {
        Self::from_vec4(v)
    }
}

impl From<Vec4A> for GVec4<f32> {
    #[inline(always)]
...
impl From<(GVec3<f32>, f32)> for Vec4A {
    #[inline(always)]
    fn from((v, w): (GVec3<f32>, f32)) -> Self {
        Self::new(v.x, v.y, v.z, w)
    }
}

impl From<(GVec2<f32>, f32, f32)> for Vec4A {
    #[inline(always)]
    fn from((v, z, w): (GVec2<f32>, f32, f32)) -> Self {
        Self::new(v.x, v.y, z, w)
    }
}

impl From<(GVec2<f32>, GVec2<f32>)> for Vec4A {
    #[inline(always)]
    fn from((a, b): (GVec2<f32>, GVec2<f32>)) -> Self {
